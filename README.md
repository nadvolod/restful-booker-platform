...
3. If running on Mac/Linux, set the permissiosn of the file first ```chmod +x script-name-here.sh```.
4. Run either ```build_locally.sh``` or ```build_locally.cmd``` depending on your OS to build RBP and get it running (It may take a while on the first run as it downloads dependencies).
5. Navigate to http://localhost:8080 to access the site
